...
export default class GaryController {
    ...
    private handleBlocked(gary: Phaser.Physics.Matter.Sprite) {
        if (this.sprite !== gary) {
            return;
        }
        this.moveTime = 0;
    ...
    private handleStomped(gary: Phaser.Physics.Matter.Sprite) {
        if (this.sprite !== gary && !this.garbage) {
            return;
        }
        this.garbage = true;
        events.off(this.name + '-stomped', this.handleStomped, this);
    ...
    private slumber() {
        ...
        if( this.stateTimer > this.stateDuration ) {
            ...
            this.stateTimer = 0;
            this.stateDuration = Phaser.Math.Between(5000,9200);
        }
    }
    ...
    private createAnims() {
        this.sprite.anims.create({
            ...
            frameRate: 2,
            repeat: -1,
            frames: this.sprite.anims.generateFrameNames('gary', {
                start: 1,
                end: 2,
                prefix: '0_Idle',
                suffix: '.webp'
