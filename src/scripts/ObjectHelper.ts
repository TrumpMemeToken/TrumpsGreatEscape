...
export function createBurger(ctx, x, y, width, height): Phaser.Physics.Matter.Sprite {
    ctx.anims.create({
        key: 'burger',
        frameRate: 5,
        frames: ctx.anims.generateFrameNumbers('burger', { start: 0, end: 3 }),
        repeat: -1
    });
    const burger = ctx.matter.add.sprite(x + (width * 0.5), y + (height * 0.5), 'burger', undefined, {
        vertices: [{ x: 0, y: 0 }, { x: 64, y: 0 }, { x: 64, y: 64 }, { x: 0, y: 64 }],
        isStatic: true,
        isSensor: true, label: 'burger'
    }).play({key: 'burger', startFrame: Phaser.Math.Between(0,3) }, true);
    burger.setData('type', 'burger');
    burger.setCollidesWith([1,2]);
    return burger;
}
