        ...
        this.sprite?.setOnCollide((data: MatterJS.ICollisionPair) => {
            ...
            if (this.obstacles.isType('exit', body)) {
                ...
                this.scene.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, () => {
                    ...
                    if( (globalThis.noWallet || (globalThis.balance == 0 && !globalThis.hasNFT)) && room !== 'bonus'  ) {
                        SceneFactory.stopSound(this.scene);
                        this.scene.scene.start( 'game-start');
                    }
                    else {
                        SceneFactory.stopSound(this.scene);
            ...
            if (this.obstacles.isType('dragon', body) ||
                ...
                this.obstacles.isType('sam', body) ||
                this.obstacles.isType('obama', body) ||
                ...
                this.obstacles.isType('hoe', body) ||
            ...
            switch (type) {
                ...
                case 'burger': {
                    events.emit('carrot-collected');
                    SceneFactory.playSound(this.sounds, 'pickupburger');
                    this.health = Phaser.Math.Clamp(this.health + 1, 0, 100);
                    this.bounceSpriteAndDestroy(sprite);
                    break;
                }
...
/*
    ...
    collectPoop(turd: Phaser.Physics.Matter.Sprite, splash = false) {
    ...
    private throwOnEnter() {
        ...
        const dropping = this.scene.matter.add.sprite(dx, dy + 8, this.projectileName, undefined, {
    ...
    public changeSkin() {
        if(globalThis.rabbitSpriteSheet === undefined) {
            globalThis.rabbitSpriteSheet = 'trump';
        }
